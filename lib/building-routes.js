...
async function loadRoutes(nestedRoutes, dir, pathBlacklist, basePath) {
    ...
    const endpointTypes = endpointRoutingPackageManager.get(ENDPOINT_TYPES_GROUP);
    ...
    for (const file of files) {
        ...
        try {
            ...
            await insertCallback(fullPath, relativePath, routePath, nestedRoutes);
        } catch (error) {
            consoleLog(`Error loading route from ${platformRespectiveFullPath}:`, error);
        }
    }
};
