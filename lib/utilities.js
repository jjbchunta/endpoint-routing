...
function resolveAndValidatePath(inputPath, { mustExist = false, mustBeJson = false, mustBeDir = false } = {}) {
    ...
    if (mustExist && !doesFileExistSync(resolvedPath)) {
    ...
    if (mustBeDir && doesFileExistSync(resolvedPath) && !fs.statSync(resolvedPath).isDirectory()) {
 ...
 * 
 * @param {String} path - The input path to resolve and validate. Needs to be absolute.
...
/**
 * Initialize a file at a specific path.
 * 
 * @param {String} path - The input path to resolve and validate. Needs to be absolute.
 */
function initializeFileSync(path) {
...
/**
 * Extract the name of a file at the suffix of a path.
 * 
 * @param {String} filePath The path to a file.
 * @throws {Error} If the provided file is not a string, an exception will be thrown.
...
/**
 * Extract the extension of a file at the suffix of a path.
 * 
 * @param {String} filePath The path to a file.
 * @throws {Error} If the provided file is not a string, an exception will be thrown.
