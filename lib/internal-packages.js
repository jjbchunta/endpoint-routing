...
endpointRoutingPackageManager.push(
    ...
    'html',
    {
        insert: handleHTMLInsert,
        interpret: handleHTMLInterpret
    }
);

/**
 * The handler behind the `insert` endpoint action for .html file types.
 */
async function handleHTMLInsert(fullPath, relativePath, routePath, routeRegistry, debug = false) {
    internalDebugNote = debug;

    let fileHandle = getFileHandle(fullPath);
    const method = formatHTTPMethod(fileHandle === 'index' ? 'get' : fileHandle);
    const nodeData = {
        filePath: relativePath,
        type: 'html',
        jointFile: false
    ...
    try {
        resolveAndValidatePath(fullPath, {mustExist: true});
    } catch(e) {
        return;
    }
...
/**
 * The handler behind the `interpret` endpoint action for .html file types.
 */
async function handleHTMLInterpret(routeNode, req, res, next = null) {
    const file = routeNode.filePath;
    res.render(file);
    return;
}