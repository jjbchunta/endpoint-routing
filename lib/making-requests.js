...
function getWithRouting() {
    this.use((req, res, next) => {
        handleMethodWithRouting('GET', req, res, next);
    });
}
...
function postWithRouting() {
    this.use((req, res, next) => {
        handleMethodWithRouting('POST', req, res, next);
    });
}
...
function putWithRouting() {
    this.use((req, res, next) => {
        handleMethodWithRouting('PUT', req, res, next);
    });
}
...
function deleteWithRouting() {
    this.use((req, res, next) => {
        handleMethodWithRouting('DELETE', req, res, next);
    });
}
 ...
 */
function handleMethodWithRouting( method, req, res, next ) {
    requestMethod = formatHTTPMethod(req.method);
    if ( requestMethod != method ) next();
    try {
        this.simulatePathRequest(req.path, requestMethod, req, res, next);
    } catch (e) {
...
async function simulatePathRequest(path, method, req, res, next = null) {
    ...
    let callback;
    try {
        callback = await this.retrieveCallbackForEndpoint(path, method);
    } catch(e) {
...
async function retrieveCallbackForEndpoint(path, method, req) {
    ...
    const _req = req || {};
    const routeNode = matchRoute(this.routingConfig.routeRegistry, path, _req);
