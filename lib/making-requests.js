...
import buildEndpointRoutes from './building-routes.js';
import {
 ...
 * @param {Object} args Optional. Additional arguments to configure how the routing occurs. Supported options are as follows:
 * 
 * * **compileOnRequest** `Bool` - When true, instead of referring to the routes inside of the compiled JSON registry, possible routes will be re-compiled on request. Intended as a developer option. By default, this is set to `false`.
 * 
 * @throws {Error} If the included `routesConfig` is not a valid path and point to a JSON file, an exception will be thrown.
 */
function initializeRouting(app, routesConfig, args = {}) {
    routesConfig = resolveAndValidatePath(
        routesConfig || 'routes.json',
        { mustExist: true, mustBeJson: true }
    );

    const compileOnRequest = extractArg(args, 'compileOnRequest', false);
    ...
    let routeRelativeDir = extractArg(routeConfig, 'relativeDir', null);
    let routeDir = extractArg(routeConfig, 'dir', null);
    let routePathBlacklist = extractArg(routeConfig, 'blacklist', []);

    const endpointRoutingConfig = {
        version: routeAlgVersion,
        routes: routeRegistry,
        relativeDir: routeRelativeDir,
        dir: routeDir,
        blacklist: routePathBlacklist,
    ...
    // Append all our custom functionality to the express instance
...
async function doesEndpointExist(path, method) {
    const routeNode = await matchRoute(this.routingConfig, path);
...
async function simulatePathRequest_iter_2(requestPath, method, req, res, next = null) {
    ...
    const routeNode = await matchRoute(this.routingConfig, requestPath, req);
    ...
    return await interpretCallback(methodRouteNode, req, res, next);
}
 ...
 *
 * @param {Object} registry The compiled JSON registry.
 * @param {String} path The request path.
 * @param {Object} [req = {}] The request object.
 * @returns {Object|null} Returns the matching route object (which should contain method keys) or null if no match.
 */
async function matchRoute(registry, path, req = {}) {
    const compileOnRequest = registry['compileOnRequest'] || false;
    let routes = registry['routes'];
    if (compileOnRequest === true) {
        // Blocking async call to keep this function synchronous
        routes = await buildEndpointRoutes({
            output: null,
            endpoints: registry['relativeDir'],
            pathBlacklist: registry['blacklist'],
            debug: false,
            returnRegistry: true
        });
